...
clf750.fit(ReshapedData2, np.hstack((0*np.ones((1,30)),1*np.ones((1,30)),2*np.ones((1,30)),3*np.ones((1,30)),4*np.ones((1,30))))[0,:])
...
#CNN.fit(np.vstack((ReshapedData3,ReshapedRan)), np.hstack((  1*np.ones((1,750)),0*np.ones((1,30)) )).T ,epochs=100 ,batch_size=30)
...
while cap.isOpened():
    ...
    #elif key%256 == 97:
        #print("a")
        #GamePlayerSet= np.array((RL_ReshapedData,))
        #PlayerHistory = np.append(PlayerHistory,HandCode)
    #elif key%256 == 98:
       # print("b")
        #GamePlayerSet= np.append(GamePlayerSet,np.array((RL_ReshapedData,)), axis=0)
        #PlayerHistory = np.append(PlayerHistory,HandCode)
       # print(GamePlayerSet)
       # print(PlayerHistory)
    #elif key%256 == 99:
    ...
    if results.multi_hand_landmarks:
        ...
        if NewHandProba > 98:
            image = cv2.putText(image, "LDA 150: " + bestcandidateSCV750  , (10,150) ,cv2.FONT_HERSHEY_SIMPLEX , 1, (255, 0, 0), 2, cv2.LINE_AA)
        else:
            image = cv2.putText(image, "LDA 150: NONE" , (10,150) ,cv2.FONT_HERSHEY_SIMPLEX , 1, (255, 0, 0), 2, cv2.LINE_AA)
        image = cv2.putText(image, "CNN Prob: " + str(np.around(NewHandProba))  , (10,100) ,cv2.FONT_HERSHEY_SIMPLEX , 1, (255, 0, 0), 2, cv2.LINE_AA)
